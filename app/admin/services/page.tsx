...
export default function ServicesManagementPage() {
  const [services, setServices] = useState<Service[]>([
    {
      ...
      shortContent: 'Building quality homes with precision and care for your family.',
      fullContent: 'Our residential construction services include custom home building, renovations, and remodeling. We work closely with homeowners to bring their vision to life.',
      photo: null,
      banner: null
    },
    {
      ...
      shortContent: 'Professional commercial construction for businesses of all sizes.',
      fullContent: 'We specialize in commercial construction projects including office buildings, retail spaces, and industrial facilities. Our team ensures timely delivery and quality workmanship.',
      photo: null,
      banner: null
    },
    {
      ...
      shortContent: 'Transform your existing space into something extraordinary.',
      fullContent: 'Our renovation and remodeling services breathe new life into your property. From kitchen and bathroom updates to complete home makeovers.',
      photo: null,
      banner: null
    }
  ]);
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>, type: 'photo' | 'banner') => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setFormData(prev => ({ 
          ...prev, 
          [type === 'photo' ? 'photoPreview' : 'bannerPreview']: reader.result as string
        }));
      };
  ...
  return (
    <div className="p-4 sm:p-6 mx-auto bg-gray-50 min-h-screen">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0 mb-6">
        <h1 className="text-lg sm:text-2xl font-semibold text-[var(--header-text)]">Services Management</h1>
        <button
      ...
      <div className="space-y-4">
        {services.map((service) => (
          <div key={service.id} className="bg-[var(--background)] border border-[var(--border-color)] rounded p-3 sm:p-4">
            {editingService === service.id ? (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm text-[var(--header-text)] mb-2">Service Photo</label>
                  ...
                  <div>
                    <label className="block text-sm text-[var(--header-text)] mb-2">Service Banner</label>
                    <div className="w-full h-32 border-2 border-dashed border-[var(--border-color)] rounded flex items-center justify-center bg-gray-50 mb-2 overflow-hidden">
                      {formData.bannerPreview ? (
                        <img src={formData.bannerPreview} alt="Banner preview" className="w-full h-full object-cover rounded" />
                      ) : (
                        <Upload className="w-6 h-6 text-gray-400" />
                      )}
                    </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm text-[var(--header-text)] mb-2">Slug</label>
                    <input
                      type="text"
                      name="slug"
                      value={formData.slug}
                ...
                <div>
                  <label className="block text-sm text-[var(--header-text)] mb-2">Short Content</label>
                  <textarea
                    name="shortContent"
                ...
                <div>
                  <label className="block text-sm text-[var(--header-text)] mb-2">Full Content</label>
                  <textarea
                    name="fullContent"
              ...
              <div className="flex flex-col sm:flex-row items-start justify-between gap-3 sm:gap-0">
                <div className="flex items-start gap-3 sm:gap-4 flex-1 w-full min-w-0">
                  <div className="w-20 sm:w-24 h-20 sm:h-24 bg-gray-100 border border-[var(--border-color)] rounded flex items-center justify-center shrink-0 overflow-hidden">
                    {service.photo ? (
                      <img src={service.photo} alt="Service" className="w-full h-full object-cover rounded" />
                    ) : (
                      <span className="text-xs text-gray-400">No Photo</span>
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-[var(--header-text)] mb-1 text-sm sm:text-base break-words">{service.heading}</h3>
                    <p className="text-xs text-gray-500 mb-2 break-all">/{service.slug}</p>
                    <p className="text-xs sm:text-sm text-gray-600 break-words">{service.shortContent}</p>
                  </div>
                </div>
